...
function preload() {
  table = loadTable("BelgiumFrance_goal_umtiti.csv","csv","header")
  console.log(table);
}
...
function draw() {
  ...
  for (var r = 0; r < rows.length; r++) {
    ...
    if (rows[r].getString("Event_Team")=="Belgium") {
      X = 100-X;
      Y = 100-Y;
    }
    ...
    if (rows[r].getNum("Sec_Before_Goal") >= -value) {
      if (rows[r].getString("Event_Tag")=="Goal" && ((X != 100 && Y != 100) && (X != 0 && Y != 0)) ) {
        stroke(color(0, 0, 0)); 
        fill(0, 0, 0);
      }
      else if (rows[r].getString("Event_Name")=="Pass") {
      ...
      else if (rows[r].getString("Event_Team")=="Belgium") {
        square(x,y,5)
      }
    }
  } 
