...
function preload() {
  table = loadTable("final_goal01_eventpos.csv","csv","header")
  console.log(table);
}
...
function draw() {
  ...
  for (var r = 0; r < rows.length; r++) {
    ...
    if (rows[r].getNum("Sec_Before_Goal") >= -value) {
      ...
      else if (rows[r].getString("Event_Name")=="Save attempt" && rows[r].getString("Event_Tag")=="Goal") {
        stroke(color(0, 0, 0)); 
        fill(0, 0, 0);
      }
